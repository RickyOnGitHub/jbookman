...
describe('Publisher e2e test', () => {
  ...
  const publisherSample = { name: 'qua per next', contact: 'definitive crossly successfully' };
  ...
  describe('new Publisher page', () => {
    ...
    it('should create an instance of Publisher', () => {
      cy.get(`[data-cy="name"]`).type('conservative');
      cy.get(`[data-cy="name"]`).should('have.value', 'conservative');

      cy.get(`[data-cy="contact"]`).type('drown the machine');
      cy.get(`[data-cy="contact"]`).should('have.value', 'drown the machine');

      cy.get(`[data-cy="website"]`).type('midst');
      cy.get(`[data-cy="website"]`).should('have.value', 'midst');

      cy.get(`[data-cy="socialmedia"]`).type('perky amidst');
      cy.get(`[data-cy="socialmedia"]`).should('have.value', 'perky amidst');
