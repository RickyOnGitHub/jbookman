...
type RestOf<T extends IAuthor | NewAuthor> = Omit<T, 'birthday'> & {
...
export class AuthorService {
  ...
  protected convertDateFromClient<T extends IAuthor | NewAuthor | PartialUpdateAuthor>(author: T): RestOf<T> {
    return {
      ...author,
      birthday: author.birthday?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAuthor: RestAuthor): IAuthor {
    return {
      ...restAuthor,
      birthday: restAuthor.birthday ? dayjs(restAuthor.birthday) : undefined,
    };
  }
